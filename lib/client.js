...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function GroupHead(props) {
      const { group, count, open, editing, onToggle, onStartRename, onCommitRename, onCancelRename, onDelete } = props
      return h('div', { className: 'workspace-groups-group-head', onClick: onToggle, role: 'treeitem', 'aria-expanded': open },
        h(Chevron, { open }),
        h('span', { className: 'workspace-groups-row-icon', }, iconOf('IconFolderOpen16', 14)),
        editing
    ...
    function Popover(props) {
      const { anchor, groups, currentGroupId, creating, onPick, onNew, onUngroup, onCreate, onClose } = props
      ...
      return h('div', {
        ...
        h('div', { className: 'workspace-groups-popover', style: { left, top }, onClick: (e) => e.stopPropagation() },
          creating ? h(CreateGroupRow, { onCreate, onCancel: onClose }) : [
            groups.map((group) => h('button', {
              ...
              h('span', { className: 'workspace-groups-pop-check' }, currentGroupId === group.id ? iconOf('IconCheckOutline16', 12) : null),
              group.name,
            )),
    ...
    function WsgBrowser(props) {
      ...
      const deleteGroup = (group) => {
        ...
        persist({ groups: doc.groups.filter((g) => g.id !== group.id), assignments: nextAssignments })
      }
      ...
      const openCreatePopover = (e) => {
        const rect = typeof e.currentTarget.getBoundingClientRect === 'function'
          ...
          : { left: 40, bottom: 60 }
        setPopover({ x: rect.left, y: rect.bottom + 4, workspaceId: null, creating: true })
      }
      ...
      const renderWorkspace = (workspace, depth) => {
          ...
          h(WorkspaceEntry, {
            ...
            onMove: (e) => {
              const rect = typeof e.currentTarget.getBoundingClientRect === 'function'
                ? e.currentTarget.getBoundingClientRect()
                : { right: 200, top: 200 }
              setPopover({ x: rect.right + 4, y: rect.top, workspaceId: workspace.workspaceId, creating: false })
            },
      ...
      if (searching) {
        ...
        for (const workspace of ungrouped) bodyRows.push(...renderWorkspace(workspace, 6))
        ...
        for (const group of groups) {
          ...
          if (groupOpen) for (const workspace of members) bodyRows.push(...renderWorkspace(workspace, 18))
        }
        if (ungrouped.length > 0) {
          ...
          if (ungroupedOpen) for (const workspace of ungrouped) bodyRows.push(...renderWorkspace(workspace, 18))
        }
      }
