  ...
  factory: (require) => {
    ...
    const TEXT = {
      ...
      en: {
        ...
        chooseDir: 'Choose workspace folder',
        homeDir: 'Home',
        parentDir: 'Up',
    ...
    // 新建分组 button: the group layers glyph + a plus badge (user request:
    // the add button must visually match the group row icon)
    const SVG_LAYERS_PLUS = '<svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linejoin="round" aria-hidden="true"><path d="M7.2 3.4 12 5.8 7.2 8.2 2.4 5.8Z"/><path d="M2.4 9.4 7.2 11.8l4.8-2.4M2.4 12.6l4.8 2.4 4.8-2.4"/><path d="M13.1 0.9v3.2M11.5 2.5h3.2" stroke-linecap="round"/></svg>'
    ...
    const CSS = `
      ...
      .workspace-groups-btn:disabled,.workspace-groups-btnPrimary:disabled{opacity:.5;cursor:default}
      ...
      /* directory-picker dialog (browse-host fallback for 新建工作区) */
      .workspace-groups-dialog{position:fixed;z-index:1201;left:50%;top:18%;transform:translateX(-50%);width:340px;max-width:92vw;background:#ffffff;color:#1f2328;border:1px solid rgba(0,0,0,.12);border-radius:12px;padding:14px 14px 12px;box-shadow:0 8px 30px rgba(20,30,50,.18);font-size:13px}
      .workspace-groups-dialog-title{font-size:14px;font-weight:600;margin:0 0 6px}
      .workspace-groups-dialog-path{font-size:12px;color:var(--dsw-alias-label-quaternary,#8a8a8a);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin:0 0 6px}
      .workspace-groups-dialog-list{max-height:240px;overflow-y:auto;border:1px solid rgba(127,127,127,.25);border-radius:8px;padding:3px;margin:0 0 8px;min-height:60px}
      .workspace-groups-dialog-list .workspace-groups-pop-item{padding:6px 8px}
      .workspace-groups-dialog-list .workspace-groups-row-icon{margin-right:6px}
      .workspace-groups-dialog-error{font-size:12px;color:#e5534b;padding:2px 4px 6px}
      ...
      @media (prefers-color-scheme: dark){
        .workspace-groups-popover,.workspace-groups-dialog{background:#22262d;color:#e6e8eb;border-color:#3c424c;box-shadow:0 8px 30px rgba(0,0,0,.5)}
        /* in dark theme the app's brand primary resolves to WHITE — a white
           primary button with dark text (the native dark primary style);
           keeping color:#fff here made 创建 invisible (user-reported) */
        .workspace-groups-btnPrimary{color:#1f2328}
      }
    `
    ...
    function WsgBrowser(props) {
      const {
        ...
        createWorkspace, pickDirectory, listDirectory, openNativeAddFlow,
      } = props
      ...
      // In-app directory picker (v0.3.3): on browse hosts the native chooser
      // is unavailable, so 新建工作区 opens this minimal browser instead of
      // hiding the button — the native browser kept its button functional on
      // the very same hosts via its in-app directory dialog.
      const [dirDialog, setDirDialog] = react.useState(null) // {stack:[path|undefined,...], entries, loading, error}
      ...
      const newSessionInWorkspace = (workspace) => {
        // Force-EXPAND the row (toggle would COLLAPSE an already-open row —
        // user-reported bug): the user must SEE the new session appear.
        if (collapsedWorkspaces[workspace.workspaceId]) {
      ...
      const scanDir = (path, stack) => {
        setDirDialog({ stack, entries: [], loading: true, error: '' })
        Promise.resolve()
          .then(() => listDirectory(path, undefined))
          .then((scan) => {
            // Wire shape (host directory-picker-browse): {path, home, entries:
            // [{name, path, hidden}], truncated} — entries are DIRECTORIES
            // only (files never reach the list), hidden marks dot-entries.
            const rawEntries = scan !== null && typeof scan === 'object' && Array.isArray(scan.entries) ? scan.entries : []
            const entries = rawEntries
              .filter((entry) => entry !== null && typeof entry === 'object' && entry.hidden !== true)
              .sort((a, b) => String(a.name).localeCompare(String(b.name), 'zh'))
            const realPath = scan !== null && typeof scan === 'object' && typeof scan.path === 'string' && scan.path !== ''
              ? scan.path
              : stack[stack.length - 1]
            const nextStack = [...stack.slice(0, -1), realPath]
            setDirDialog((prev) => (prev === null ? prev : { ...prev, stack: nextStack, entries, loading: false }))
          })
          .catch((error) => {
            setDirDialog((prev) => (prev === null ? prev : { ...prev, loading: false, error: String((error && error.message) || error) }))
          })
      }
      const openDirDialog = () => scanDir(undefined, [undefined])
      const enterDir = (entry) => {
        if (dirDialog === null || dirDialog.loading) return
        const stack = [...dirDialog.stack, entry.path]
        scanDir(entry.path, stack)
      }
      const upDir = () => {
        if (dirDialog === null || dirDialog.loading || dirDialog.stack.length <= 1) return
        const stack = dirDialog.stack.slice(0, -1)
        scanDir(stack[stack.length - 1], stack)
      }
      const confirmDir = () => {
        if (dirDialog === null || addBusy) return
        const path = dirDialog.stack[dirDialog.stack.length - 1]
        if (!path) return
        setAddBusy(true)
        Promise.resolve()
          .then(() => createWorkspace({ path: String(path) }))
          .then(() => { setDirDialog(null); setDocError('') })
          .catch((error) => {
            setDirDialog((prev) => (prev === null ? prev : { ...prev, error: `${T('addWorkspaceFailed')}: ${String(error)}` }))
          })
      ...
      const addWorkspace = () => {
        if (addBusy) return
        // Native-first (v0.3.4): launch the host's own add-workspace flow —
        // it composes the browse/native backend itself and renders the full
        // native dialog on every host kind.
        try {
          if (openNativeAddFlow() === true) return
        } catch (error) { guard('openNativeAddFlow')(error) }
        // Browse hosts have no native chooser — open the in-app directory
        // dialog directly (the shipped browser keeps its own add button
        // functional on these hosts the same way). Never hide the button.
        ...
        // backend refusal switches this page to the in-app dialog.
        Promise.resolve()
          .then(() => pickDirectory())
          .then((path) => {
          ...
          .then((created) => {
            if (created !== undefined) { setDirDialog(null); setDocError('') }
          })
          .catch((error) => {
      ...
      return h('div', { className: 'workspace-groups-outer' },
        h('div', { className: 'workspace-groups-root' },
          h('div', { className: 'workspace-groups-header' },
            ...
            }, h('span', { className: 'workspace-groups-row-icon', dangerouslySetInnerHTML: { __html: SVG_LAYERS_PLUS } })),
            h('button', {
              type: 'button',
              className: 'workspace-groups-icon-btn',
          ...
          dirDialog !== null ? h('div', {
            className: 'workspace-groups-popover-back',
            onClick: () => setDirDialog(null),
            onContextMenu: (e) => { e.preventDefault(); setDirDialog(null) },
          },
            h('div', { className: 'workspace-groups-dialog', onClick: (e) => e.stopPropagation() },
              h('div', { className: 'workspace-groups-dialog-title' }, T('chooseDir')),
              h('div', { className: 'workspace-groups-dialog-path', title: dirDialog.stack[dirDialog.stack.length - 1] || '' }, dirDialog.stack[dirDialog.stack.length - 1] || T('homeDir')),
              h('div', { className: 'workspace-groups-dialog-list' },
                dirDialog.loading
                  ? h('div', { className: 'workspace-groups-empty' }, T('loading'))
                  : dirDialog.entries.length === 0
                    ? h('div', { className: 'workspace-groups-empty' }, T('noSubDirs'))
                    : dirDialog.entries.map((entry) => h('button', {
                        key: entry.path,
                        type: 'button',
                        className: 'workspace-groups-pop-item',
                        onClick: () => enterDir(entry),
                      },
                        h('span', { className: 'workspace-groups-row-icon' }, iconOf('IconFolderClose16', 13)),
                        entry.name,
                      )),
              ),
              dirDialog.error !== '' ? h('div', { className: 'workspace-groups-dialog-error' }, dirDialog.error) : null,
              h('div', { className: 'workspace-groups-pop-btnrow' },
                h('button', { type: 'button', className: 'workspace-groups-btn', disabled: dirDialog.loading || dirDialog.stack.length <= 1, onClick: upDir }, T('parentDir')),
                h('button', { type: 'button', className: 'workspace-groups-btn', onClick: () => setDirDialog(null) }, T('cancel')),
                h('button', {
                  type: 'button',
                  className: 'workspace-groups-btn workspace-groups-btnPrimary',
                  disabled: addBusy || dirDialog.loading || !dirDialog.stack[dirDialog.stack.length - 1],
                  onClick: confirmDir,
                }, T('useThisDir')),
              ),
            ),
      ...
      try {
        ...
        if (slots !== null && typeof slots.inject === 'function' && typeof slots.register === 'function') {
          ...
          const actions = () => ({
            ...
            openNativeAddFlow: () => mountNativeAddFlow(),
          })
          // 新建工作区, native-first (v0.3.4): launch the host's OWN
          // add-workspace flow — the current registrant of the
          // `sidebar.workspaces.directoryFlow` hole (the shell's picker,
          // which composes browse/native per host capability and renders the
          // full native dialog: breadcrumbs, create-folder, i18n). The flow
          // conversation contract mirrors the shipped WorkspacePickFlow's
          // owner side: {open, busy, onPicked(path), onCancel, onError}
          // merged with the registrant's own injected services. Returns
          // false when no flow is registered (caller falls back).
          let flowPortal = null
          let flowRoot = null
          const unmountNativeFlow = () => {
            try { if (flowRoot !== null) flowRoot.unmount() } catch { /* already gone */ }
            flowRoot = null
            if (flowPortal !== null) { flowPortal.remove(); flowPortal = null }
          }
          const mountNativeAddFlow = () => {
            let entries = []
            try { entries = slots.entries('sidebar.workspaces.directoryFlow') } catch { entries = [] }
            const entry = entries.length > 0 ? entries[entries.length - 1] : null
            if (entry === null || typeof entry.component !== 'function') return false
            if (flowPortal === null) {
              flowPortal = document.createElement('div')
              flowPortal.dataset.dshPlugin = 'workspace-groups'
              document.body.appendChild(flowPortal)
              flowRoot = createRoot(flowPortal)
            }
            const injected = typeof entry.inject === 'function'
              ? entry.inject()
              : (entry.inject !== null && typeof entry.inject === 'object' ? entry.inject : {})
            const owner = {
              open: true,
              busy: false,
              onPicked: (path) => {
                Promise.resolve()
                  .then(() => ctx.get('workspaces').create({ path: String(path) }))
                  .then(() => { unmountNativeFlow() })
                  .catch((error) => { console.error('[workspace-groups] native flow create failed:', error); unmountNativeFlow() })
              },
              onCancel: () => { unmountNativeFlow() },
              onError: (message) => { console.error('[workspace-groups] native flow error:', message); unmountNativeFlow() },
            }
            flowRoot.render(h(entry.component, Object.assign({}, injected, owner)))
            return true
          }
          try {
