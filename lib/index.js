...
function sanitizeState(input) {
  ...
  for (const group of rawGroups) {
    ...
    ids.add(group.id)
    // parentId is optional (multi-level grouping, v0.3.0): validated after all
    // ids are known — a dangling/self/cyclic parent is stripped (demoting the
    // group to top level) instead of rejecting the whole document.
    const groupOut = { id: group.id, name: group.name }
    if (group.parentId !== undefined) {
      if (typeof group.parentId !== 'string' || group.parentId === '' || group.parentId.length > MAX_ID) return undefined
      groupOut.parentId = group.parentId
    }
    groups.push(groupOut)
  }
  // Resolve parent references now that every id is known.
  for (const group of groups) {
    if (group.parentId === undefined) continue
    let bad = group.parentId === group.id || !ids.has(group.parentId)
    if (!bad) {
      // Walk to the root; a revisited id means a cycle.
      const seen = new Set([group.id])
      let cursor = group.parentId
      while (cursor !== undefined) {
        if (seen.has(cursor)) { bad = true; break }
        seen.add(cursor)
        const parent = groups.find((g) => g.id === cursor)
        cursor = parent !== undefined && parent.parentId !== undefined ? parent.parentId : undefined
      }
    }
    if (bad) delete group.parentId
  }
